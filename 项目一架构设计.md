...
文档状态：开发基线（含待审核决策）
编制日期：2026-09-21  
需求基线：[项目一需求规格说明.md](./项目一需求规格说明.md) V1.1
...
本版本直接补齐不会改变产品范围的架构缺口：领域不变量、播放会话契约、手动建段入口、识别产物元数据、任务恢复检查点、录音中断状态和统一帧时基。

本次已确认的产品规则：任何低置信度识别句段都必须进入人工审核。审核前允许用户修改、试听和继续编辑，但不得作为已审核结果导出或发布；审核结论和所依据的文字/边界修订号必须持久化。具体阈值由版本化审核策略提供，不能在未校准前擅自写死一个数值。

以下事项仍需产品或技术负责人审核后才能成为首版承诺：

1. `XiaoyiTimedAlignmentAdapter` 是否有 API 20 可用、可分发且完全离线的正式实现；若不可用，是否引入本地对齐模型或调整 P1-FR-006 的首版验收。
2. 首版导入格式和导出格式矩阵，以及是否把 PCM/WAV 作为唯一首版格式。
3. 首版后台识别/渲染/导出的继续策略，以及这些策略对应的电量和通知要求。

已确认并写入本版本的规则：`DRAFT_READY` 允许追加录音。界面必须提供两个明确选项：`CONTINUE_ORIGINAL_SESSION`（继续原录音会话）和 `START_NEW_SESSION`（新建录音会话）。继续原会话时，系统在原草稿之后追加有效帧，并通过新的封存资产或新的草稿修订表达，不覆盖旧草稿；新建会话时保留旧草稿，创建新的录音会话和 staging 资产。两种模式都遵守源录音不可变原则。

淡入淡出默认值已确定为代码中的版本化 `DspProfile` 默认项，当前默认淡入和淡出均为 10,000 微秒（10 ms），且不得超过有效内容时长的一半。后续可以修改默认配置；已保存修订必须记录 `dspProfileVersion`，预览和导出使用同一版本，修改默认值不能悄悄改变历史修订的输出。
...
| `TranscriptEditing` | 自动分句、改字、合并、拆分、边界调整、低置信标记、波形手动建段 | P1-FR-007—010、020 |
...
│   │   ├── ReviewTranscriptUseCase.ets
│   │   ├── CreateManualSegmentUseCase.ets
...
interface RecognitionArtifact {
  ...
  timestampCapability: 'WORD' | 'PHRASE' | 'NONE';
  confidenceCapability: 'PROVIDED' | 'UNAVAILABLE' | 'DERIVED_WITH_LIMITATION';
  confidenceReviewPolicyVersion: string;
...
interface ConfidenceReviewPolicy {
  version: string;
  lowConfidenceThreshold?: number;
  missingThresholdAction: 'REQUIRE_REVIEW';
  unavailableConfidenceAction: 'REQUIRE_REVIEW';
  derivedConfidenceAction: 'REQUIRE_REVIEW';
}
...
interface TranscriptSegment {
  ...
  sourceRange: TimeRangeUs;
  confidence?: number;
  reviewStatus: 'NOT_REQUIRED' | 'PENDING_HUMAN_REVIEW' | 'APPROVED';
  reviewedTextRevision?: number;
  reviewedRangeRevision?: number;
  reviewedPolicyVersion?: string;
  reviewedBy?: 'USER';
  reviewedAtMs?: number;
...
interface ProjectEditRevision {
  ...
  recognitionArtifactHash?: string;
  reviewGate: 'CLEAR' | 'HUMAN_REVIEW_REQUIRED';
...
interface OperationCheckpoint {
  sessionId: string;
  operation: 'ACQUIRE_SOURCE' | 'RECOGNIZE' | 'RECORD_TAKE' | 'RENDER' | 'EXPORT';
  phase: 'STARTED' | 'RUNNING' | 'CANCELLING' | 'STAGED' | 'COMMITTED' | 'FAILED';
  startedRevisionId?: RevisionId;
  expectedHeadRevisionId?: RevisionId;
  sourceContentHash?: string;
  stagedAssetRelativePath?: string;
...
interface ExportRecord {
  ...
  revisionId: RevisionId;
  manifestHash: string;
  status: 'STAGED' | 'PUBLISHED' | 'FAILED' | 'CANCELLED';
  targetUri?: string;
...
`WordToken` 保存模型原始文字和源时间坐标，不随用户改字、合并或拆分而覆盖。用户最终看到的内容保存在 `TranscriptSegment.displayText`。`tokenIds` 可以为空，仅限手动句段；此时 `boundaryOrigin` 必须为 `MANUAL`，显示文字由用户输入或为空，并且必须仍有合法的 `sourceRange`。

置信度数值使用可选字段，但能力状态必须记录在 `RecognitionArtifact.confidenceCapability`。适配器缺少该字段时标记为 `UNAVAILABLE`，不得自行生成数值。`ConfidenceReviewPolicy` 是版本化配置：当能力为 `PROVIDED` 且数值低于 `lowConfidenceThreshold` 时，句段必须为 `PENDING_HUMAN_REVIEW`；当阈值未配置、能力为 `UNAVAILABLE` 或 `DERIVED_WITH_LIMITATION` 时，按保守策略同样必须为 `PENDING_HUMAN_REVIEW`，不能因为缺少数值或配置而绕过审核。只有人工确认当前文字和源边界后，才能转为 `APPROVED`。人工审核记录必须绑定当时的 `textRevision`、`rangeRevision`、`reviewedPolicyVersion` 和审核时间；文字或边界再次变化后自动退回 `PENDING_HUMAN_REVIEW`。

`ProjectEditRevision.reviewGate` 是修订级硬门槛：只要任一句段处于 `PENDING_HUMAN_REVIEW`，就必须为 `HUMAN_REVIEW_REQUIRED`。`RenderPlanCompiler` 可以生成预览清单供审核使用，但正式导出和发布前必须校验 `reviewGate === 'CLEAR'`；否则返回 `HUMAN_REVIEW_REQUIRED`，不得生成 `PUBLISHED` 的 `ExportRecord`。人工审核用例只能确认或退回句段，不能直接修改源 token；修改文字或边界必须走普通编辑用例并重新触发审核。

审核状态只允许沿以下路径变化：`NOT_REQUIRED → NOT_REQUIRED`（高于已配置阈值且能力可靠）、`PENDING_HUMAN_REVIEW → APPROVED`（用户确认文字和边界）、`PENDING_HUMAN_REVIEW → PENDING_HUMAN_REVIEW`（用户退回或尚未处理）、`APPROVED → PENDING_HUMAN_REVIEW`（文字、边界、识别产物或审核策略版本变化）。`APPROVED` 必须同时写入 `reviewedBy: 'USER'`、`reviewedTextRevision`、`reviewedRangeRevision`、`reviewedPolicyVersion` 和 `reviewedAtMs`；缺少任一字段，或 `reviewedPolicyVersion` 与当前识别修订采用的审核策略版本不一致，都不能通过修订校验。

`TransitionDecision` 为每个未被句段覆盖的源间隙保存决策。普通句间区间为 `[left.endUs, right.startUs)`；首句前区域使用 `[0, first.startUs)`，末句后区域使用 `[last.endUs, sourceDurationUs)`，分别以缺省的 `leftSegmentId` 或 `rightSegmentId` 表示边界。没有任何句段时直接输出完整源音频，不要求创建转场记录。`PRESERVE_SOURCE` 输出该源间隙且 `fixedSilenceUs` 必须为 `0`，`FIXED_SILENCE` 输出 `fixedSilenceUs > 0` 的静音，二者只能选择其一，不能同时保留源间隙和固定静音。合并或拆分句段后，按新的相邻关系重建受影响的转场记录；未受影响的转场沿用原记录。转场模式、静音时长和边界版本属于修订状态，不能只保存在 UI 或渲染缓存中。
...
- 合并句段时，新句段的 token 范围是参与句段 token 范围的有序并集；手动句段不能伪造 token，合并后的 `tokenIds` 可以为空但必须保留来源句段链路。
- 拆分只能发生在 token 边界、用户明确选择的时间点或字符边界；拆分结果的 sourceRange 必须互不重叠且覆盖原句段范围。
- 每个修订中 `order` 必须唯一、连续，并严格按 `sourceRange.startUs` 排序；任何违反都拒绝提交。
- 每个可渲染源间隙必须恰好关联一个 `TransitionDecision`（无句段时例外）；缺失、重复或与相邻句段边界不一致的转场记录都拒绝提交。
- 识别提交时依据 `ConfidenceReviewPolicy` 初始化每个句段的 `reviewStatus` 和修订级 `reviewGate`；任何低置信度、置信度不可用或仅为受限推导的句段都不得初始化为 `APPROVED`。
- 审核通过只对审核时的 `textRevision` 和 `rangeRevision` 有效；任一版本变化、句段合并/拆分或识别产物变化都会使相关句段重新进入 `PENDING_HUMAN_REVIEW`，并将修订级 `reviewGate` 置为 `HUMAN_REVIEW_REQUIRED`。
- 审核通过还必须匹配当前识别修订的审核策略版本；策略版本变化时，所有受该策略影响的已审核句段重新进入 `PENDING_HUMAN_REVIEW`，并将修订级 `reviewGate` 置为 `HUMAN_REVIEW_REQUIRED`。
- `reviewGate` 为 `HUMAN_REVIEW_REQUIRED` 时允许保存、预览和继续编辑，但导出提交必须失败并返回 `HUMAN_REVIEW_REQUIRED`；只有全部需要审核的句段均为 `APPROVED` 或 `NOT_REQUIRED` 时才可置为 `CLEAR`。
...
interface AssetFrameMapping {
  ...
  rounding: 'CUMULATIVE_RATIONAL_FLOOR';
}
```

导出开始时固定 `editRevisionId` 和 `manifestHash`，并再次校验 `reviewGate === 'CLEAR'`。用户随后继续编辑，只会生成新的修订，不改变已经运行的导出任务；审核状态变化或修订冲突不得覆盖已启动导出任务。
...
compile(revision):
  ...
  1a. 校验 reviewGate；预览模式允许输出带审核提示的清单，导出模式若为 HUMAN_REVIEW_REQUIRED 则返回错误
  ...
  5. 按顺序处理每个句段：
       ...
       c. 对首句前、句间和末句后区域读取唯一的 TransitionDecision，选择保留源间隙或输出用户设置的固定静音
  6. 输出末句之后的 SourceAudio[last.end, sourceDuration)
  7. 合并连续、同源且参数一致的节点
  8. 使用每个资产的 `AssetFrameMapping` 换算到统一输出格式，按有理数累计并向下取整；同一源边界只换算一次
...
淡入淡出默认值来自版本化配置，当前基线为：

```ts
interface DspProfile {
  version: number;
  defaultFadeInUs: number;  // 10000
  defaultFadeOutUs: number; // 10000
  maxFadeRatio: number;     // 0.5
}
```

用户或产品后续可以调整默认配置；编辑修订必须持久化实际使用的 `dspProfileVersion` 和参数快照。实现必须限制淡入和淡出不超过有效内容时长的一半。响度匹配的测量值、算法版本和最终增益写入编辑修订。源文件和补录原件均不进行覆盖式处理。

帧时基规则固定如下：持久化时间使用微秒，资产内部使用原生帧，渲染计划使用目标采样率帧。时间到帧转换采用累计有理数向下取整，边界表在一次编译中复用；相邻源范围转换后必须满足前一段 `endFrame <=` 后一段 `startFrame`，覆盖范围不得产生重叠或空洞。淡入淡出只改变节点包络，不改变节点的帧区间；如后续需要跨节点交叉淡化，必须新增版本化规则并重新验证总帧数与预览/导出一致性。
...
interface AudioCapturePort {
  ...
  flush(sessionId: string): Promise<AppResult<{ validFrameCount: number }>>;
...
interface PlaybackSession {
  ...
  play(): Promise<AppResult<void>>;
  pause(): Promise<AppResult<void>>;
  seek(positionFrame: number): Promise<AppResult<void>>;
  stop(): Promise<AppResult<void>>;
  onProgress(observer: (positionFrame: number) => void): void;
  onEnded(observer: () => void): void;
  onError(observer: (error: AppError) => void): void;
}
```
...
端口契约补充：文件选择器返回的 URI/FD 只在选择回调和适配器内部短暂有效。`AudioImportPort` 必须在回调内把内容复制到项目私有 `staging`，完成摘要和探测后关闭 FD；领域层和修订只保存私有相对路径及摘要，不保存外部 URI/FD 作为音频资产来源。导出先写私有临时文件，关闭并完成可解码验证、摘要后再通过 `DocumentViewPicker` 返回的目标 URI 发布；发布结果写入 `ExportRecord`。URI/FD 失效、权限撤回、复制中断或发布失败必须返回明确错误并保留可清理的 staging 记录，重试不得重复提交成功记录。

`flush(sessionId)` 必须把当前可读数据写入 staging，返回最后一个有效帧数，并保证后续 `stop` 或进程恢复不会把未完成的尾部帧当作有效音频。`CapturedDraft` 至少包含 staging 相对路径、格式元数据、有效帧数和可解码性校验结果。
...
录音增加 `PAUSED`、`INTERRUPTED`、`DRAFT_READY`。用户主动暂停可以 `PAUSED → RUNNING` 继续同一会话；系统中断先进入 `INTERRUPTED`，只有草稿通过可解码性和元数据校验后才能进入 `DRAFT_READY`。从 `DRAFT_READY` 追加录音时，用户必须选择继续原录音会话或新建录音会话：前者追加到新的草稿修订，后者创建新的录音会话和 staging 资产；旧草稿始终保留且不被覆盖。导出状态不写入项目主阶段，导出失败后项目仍为 `EDITABLE`。
...
- 长任务提交必须使用 `commit(projectId, expectedHeadRevisionId, newRevision)` 进行比较并交换（CAS）；若当前 `HEAD` 已变化，提交返回 `REVISION_CONFLICT`，任务结果保留为未提交产物，不得覆盖用户较新的编辑。
...
- 识别、渲染和导出都必须写入 `OperationCheckpoint`；进程被杀后只能恢复到最近一个已提交修订或明确可验证的 staging 草稿，不得把未提交结果当作成功。
- 同一设备的识别模型会话和 PCM 渲染任务使用有界并发配额；生产者在队列满时暂停读取，取消后等待适配器确认资源释放。

生命周期行为固定如下；“继续”仅表示从检查点或已封存草稿重新启动，不表示继续使用已失效的系统对象：

| 操作 | Ability 销毁/页面离开 | 进入后台 | 进程终止后恢复 |
| --- | --- | --- | --- |
| 录音 | 停止并 `flush`，封存可解码草稿 | 停止并封存 `DRAFT_READY` | 从草稿校验后进入待确认状态 |
| 识别 | 取消 UI 订阅，任务按协调器策略继续或取消 | 继续受控任务并更新检查点 | 从最近检查点重试；未提交结果不得直接提交 |
| 播放 | 停止播放器并保存 `PlaybackCursor` | 暂停并保存游标 | 按用户操作恢复到保存位置，修订不匹配时丢弃游标 |
| 渲染 | 取消或继续后台任务，释放页面订阅 | 继续并更新处理帧 | 从最近安全边界重建清单并重新渲染 |
| 导出 | 取消或继续后台任务，保留 `ExportRecord` | 继续并更新检查点 | 校验 staging 与 manifest；幂等发布或清理 |
...
1. 取得项目级写锁，并读取当前 `HEAD`；
2. 在内存生成下一修订，校验全部领域不变量并编译一次渲染清单；
3. 写入 `revision-xxxxxx.json.tmp`，关闭并计算摘要；
4. 把临时文件改名为正式修订；
5. 仅当 `HEAD` 仍等于调用方的 `expectedHeadRevisionId` 时写入新的 `HEAD.tmp`，再替换 `HEAD`；否则返回 `REVISION_CONFLICT` 并保留未提交修订文件供恢复器清理；
6. 释放写锁。
...
- 任意编辑序列前后源音频摘要保持不变。
- 首句前、句间和末句后转场分别覆盖且不重复；固定静音不会同时保留源间隙；合并/拆分只迁移受影响的转场。
- 不同原生采样率资产的 `AssetFrameMapping` 在边界处无重叠、无空洞，累计取整后预览和导出总帧数一致。
...
- 替换文件缺失时明确回退源录音；
- 手动波形建段的空 token、边界校验、排序和提交恢复；
- `PlaybackSession` 的进度、高亮、暂停、seek、结束和错误回调；
- 识别产物模型版本、源摘要和原始结果恢复；检查点哈希校验与幂等恢复；
- 低置信度句段自动进入 `PENDING_HUMAN_REVIEW`；审核通过后才允许清除 `reviewGate`；文字或边界修改会重新触发审核；审核未完成时导出被拒绝；
- 审核记录持久化 `reviewedPolicyVersion`；审核策略版本变化后旧审核不能继续清除 `reviewGate`；
- CAS 版本冲突不会覆盖新编辑；
- 导入 URI/FD 失效、导出发布失败、重试和 staging 幂等清理；
...
`XiaoyiTimedAlignmentAdapter` 的确认不能只看类名或宣传页，必须在 HarmonyOS SDK 6.0.0（API 20）目标工程和目标真机完成离线契约测试，并取得可引用的官方 API 文档或厂商确认。测试记录至少包括：正式包名、类名和方法签名；是否接受本地 PCM/WAV；时间戳粒度（词级、短语级或句级）；置信度字段或等价不确定性；普通话 5 分钟和 20 分钟输入；飞行模式；取消、进程中断、重复识别和明确错误码；模型版本、获取方式、许可证与离线分发限制；设备和系统版本；耗时与峰值内存。如果只能返回纯文本、不能返回可定位时间范围，不能宣称满足 P1-FR-006；如果没有置信度，按现行规则全部进入人工审核，但仍需单独解决时间定位。测试结果在这些证据齐全前保持为技术阻断项。
...
| P1-FR-006—010 | `CoreSpeechTranscriptAdapter`、`XiaoyiTimedAlignmentAdapter`、`TranscriptSegmentService`、句段编辑模型、`ReviewTranscriptUseCase` 与 `reviewGate` |
...
| P1-AC-006—009、013 | 时间轴领域不变量、预览/导出一致性和低置信度人工审核门槛测试 |
...
| ADR-009 | 低置信度句段强制人工审核 | 识别不确定性不能直接进入可发布音频；审核状态绑定文字和边界版本，导出前以修订级门槛阻断 |
...
- 低置信度句段在审核完成前无法生成 `PUBLISHED` 导出记录，审核通过且版本未变化后才可导出；
- `APPROVED` 句段的审核策略版本与当前修订不一致时无法生成 `PUBLISHED` 导出记录；
